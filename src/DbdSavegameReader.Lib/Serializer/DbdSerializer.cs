...
public static class DbdSerializer
{
    ...
    public static string ReadJson(string fileContent)
    {
        ...
        var magicOuter = fileContent[4..12];
        ...
        // Decrypt payload
        var payload = fileContent[12..^1];
        var rawPayload = Crypto.Base64Decode(payload);
        var decryptedPayload = Crypto.Decrypt(rawPayload, EncryptionKey);
    ...
    {
        return JsonSerializer.Deserialize<DbdSavegame>(json, SerializerOptions) ?? throw new SerializationException("Failed to deserialize JSON.");
    }
}